...
Defender_Observation = TypedDict('Defender_Observation', {'prev_infected_nodes': np.ndarray,
                                                          'prev_incoming_firewall_status':np.ndarray,
                                                          'prev_outgoing_firewall_status':np.ndarray,
                                                          'prev_services_status':np.ndarray,
                                                          'infected_nodes': np.ndarray,
...
class DefenderEnvWrapper(gym.Env, IEnvironmentObserver):
    ...
    def __create_observation_space(self, cyber_env: CyberBattleEnv) -> gym.Space:
        ...
        return spaces.Dict({'prev_infected_nodes': spaces.MultiBinary(len(list(cyber_env.environment.network.nodes))),
                            'prev_incoming_firewall_status': spaces.MultiBinary(len(self.firewall_rule_list)*len(list(cyber_env.environment.network.nodes))),
                            'prev_outgoing_firewall_status': spaces.MultiBinary(len(self.firewall_rule_list)*len(list(cyber_env.environment.network.nodes))),
                            'prev_services_status': spaces.MultiBinary(self.num_services),
                            'infected_nodes': spaces.MultiBinary(len(list(cyber_env.environment.network.nodes))),
    ...
    def step(self, action) -> Tuple[Observation, float, bool, Dict[str, Any]]:
        ...
        if self.delay:
            self.queue.append(copy.copy(defender_observation))
            if self.timesteps > self.delay:
                defender_observation = self.queue[0]
                self.queue.pop(0)        
        ...
        self.network_availability.append(self.cyber_env.network_availability)
        self.network_infectivity.append(self.cyber_env.network_infectivity)
    ...
    def get_blank_defender_observation(self):
        ...
        obs = Defender_Observation(prev_infected_nodes = [],
                                    prev_incoming_firewall_status=[],
                                    prev_outgoing_firewall_status=[],
                                    prev_services_status=[],
                                   infected_nodes = [],
    ...
    def observe(self) -> Defender_Observation:
        ...
        if self.observation:
            new_observation["prev_infected_nodes"] = self.observation["infected_nodes"]
            new_observation["prev_incoming_firewall_status"] = self.observation["incoming_firewall_status"]
            new_observation["prev_outgoing_firewall_status"] = self.observation["outgoing_firewall_status"]
            new_observation["prev_services_status"] = self.observation["services_status"]
        ...
        if not np.size(self.observation['prev_infected_nodes']):
            new_observation["prev_infected_nodes"] = new_observation["infected_nodes"]
            new_observation["prev_incoming_firewall_status"] = new_observation["incoming_firewall_status"]
            new_observation["prev_outgoing_firewall_status"] = new_observation["outgoing_firewall_status"]
            new_observation["prev_services_status"] = new_observation["services_status"]
