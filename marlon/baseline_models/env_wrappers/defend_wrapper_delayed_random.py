...
Defender_Observation = TypedDict('Defender_Observation', {'timestep': np.ndarray,
...
class DefenderEnvWrapper(gym.Env, IEnvironmentObserver):
    ...
    def __create_observation_space(self, cyber_env: CyberBattleEnv) -> gym.Space:
        ...
        return spaces.Dict({'timestep': spaces.Box(low=0, high=100, shape=(1,), dtype=np.float32),
    ...
    def step(self, action) -> Tuple[Observation, float, bool, Dict[str, Any]]:
        ...
        if (self.timesteps > 2 and val == 0):
            defender_observation = copy.copy(self.last_obs)
        else:
            self.last_obs = defender_observation

        
            
        #defender_observation['timestep'] = self.timesteps - defender_observation['timestep']
        time_value = self.timesteps - defender_observation['timestep']
        defender_observation = copy.copy(defender_observation)
        defender_observation['timestep'] = time_value
        self.timesteps += 1
    ...
    def get_blank_defender_observation(self):
        ...
        obs = Defender_Observation(timestep=0,
                                   infected_nodes = [],
    ...
    def observe(self) -> Defender_Observation:
        ...
        new_observation['timestep'] = self.timesteps
