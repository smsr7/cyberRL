...
def train(step=0, evaluate_after=False, max_episodes=20, defense_start=False, first=True):
    if step < max_episodes:
        if (defense_start) and (step == 0):
            ...
            if step == 1:
                ...
                defender_builder=LoadFileBaselineAgentBuilder(
                        alg_type=PPO,
                        file_path=f'{SAVE_DIR}/{step-1 if first and (step % 2 != 1) else step-2}_{DEFENDER_SAVE_PATH}'
                        ),
                attacker_invalid_action_reward_modifier=ATTACKER_INVALID_ACTION_REWARD_MODIFIER,
                attacker_invalid_action_reward_multiplier=ATTACKER_INVALID_ACTION_REWARD_MULTIPLIER,
                defender_invalid_action_reward_modifier=DEFENDER_INVALID_ACTION_REWARD,
        ...
        if first:  
            first = False
            train(step=step, evaluate_after=True, max_episodes=10, defense_start=False, first=False)
        else:
            first = True
            train(step=step+1, evaluate_after=True, max_episodes=10, defense_start=False, first=True)
