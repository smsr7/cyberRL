...
SAVE_DIR='model_saves/ladder_20240529_164017'
EVALUATE_EPISODES = 5
...
def evaluate(step=1, max_steps=10, attacker=True):
    if step<max_steps:
        universe = MultiAgentUniverse.build(
            env_id='CyberBattleToyCtf-v0',
            attacker_builder=LoadFileBaselineAgentBuilder(
                    alg_type=PPO,
                    file_path=f'{SAVE_DIR}/{step if attacker else max_steps-1}_{ATTACKER_SAVE_PATH}'
                    ),
            defender_builder=LoadFileBaselineAgentBuilder(
                    alg_type=PPO,
                    file_path=f'{SAVE_DIR}/{max_steps-1 if attacker else step}_{DEFENDER_SAVE_PATH}'
                    ),
        ...
        pd.DataFrame(data=[out], columns=columns).to_csv(f'{SAVE_DIR}/out_best_{"attacker" if attacker else "defender"}.csv', mode='a', header=not os.path.exists(f'{SAVE_DIR}/out.csv'), index=False)
        evaluate(step=step+1, attacker=attacker)

if __name__ == "__main__":
    evaluate(attacker=False)
