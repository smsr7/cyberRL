...
SAVE_DIR='model_saves/ladder_20240613_135239'
EVALUATE_EPISODES = 5
...
def evaluate(step=0, episode=0, max_steps=5, max_episodes=20, defense_start=True):
    if episode<max_episodes:
        if episode == 0:
            universe = MultiAgentUniverse.build(
                ...
                defender_builder=LoadFileBaselineAgentBuilder(
                        alg_type=PPO,
                        file_path=f'{SAVE_DIR}/{episode}_{step}_{DEFENDER_SAVE_PATH}'
                        ),
            ...
            universe = MultiAgentUniverse.build(
                ...
                attacker_builder=LoadFileBaselineAgentBuilder(
                        alg_type=PPO,
                        file_path=f'{SAVE_DIR}/{episode if (episode%2 == 1) else episode-1}_{step if (episode % 2 == 1) else max_steps}_{ATTACKER_SAVE_PATH}'
                        ),
                defender_builder=LoadFileBaselineAgentBuilder(
                        alg_type=PPO,
                        file_path=f'{SAVE_DIR}/{episode if (episode%2 == 0) else episode-1}_{step if (episode % 2 == 0) else max_steps}_{DEFENDER_SAVE_PATH}'
                        ),
                attacker_invalid_action_reward_modifier=ATTACKER_INVALID_ACTION_REWARD_MODIFIER,
                attacker_invalid_action_reward_multiplier=ATTACKER_INVALID_ACTION_REWARD_MULTIPLIER,
                defender_invalid_action_reward_modifier=DEFENDER_INVALID_ACTION_REWARD,
        ...
        if step<max_steps:
            evaluate(episode=episode, step=step+1, max_episodes=max_episodes, max_steps=max_steps)
        else:
            evaluate(episode=episode+1, step=0, max_episodes=max_episodes, max_steps=max_steps)
