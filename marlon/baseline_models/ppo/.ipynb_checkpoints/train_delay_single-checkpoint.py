...
def train(episode=0, step=0, evaluate_after=True, max_episodes=3, max_steps=2, defense_start=True, lag=False, delay=0):
    ...
    if episode < max_episodes:
        ...
        if step<max_steps:
            train(episode=episode, step=step+1, evaluate_after=True, max_episodes=max_episodes, max_steps=max_steps, delay=delay)
        else:
            train(episode=episode+1, step=0, evaluate_after=True, max_episodes=max_episodes, max_steps=max_steps, delay=delay)

if __name__ == '__main__':
#    for i in range(1, 5):
    train(max_episodes=20, max_steps=5, defense_start=True, lag=False, delay=0)
