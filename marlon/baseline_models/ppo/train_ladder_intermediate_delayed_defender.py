...
def train(episode=0, step=0, evaluate_after=True, max_episodes=3, max_steps=2, defense_start=True, lag=0.6):
        ...
        if (defense_start) and (episode == 0):
            ...
            if episode == 1:
                ...
                if (episode%2 == 1):
                    ...
                    attacker_builder=LoadFileBaselineAgentBuilder(
                            alg_type=PPO,
                            file_path=f'{SAVE_DIR}/{episode-3 if (step / max_steps) <= lag else episode-1}_{max_steps}_{ATTACKER_SAVE_PATH}'
                            ) if (episode > 3) else RandomAgentBuilder(),
                    defender_builder=LoadFileBaselineAgentBuilder(
                            alg_type=PPO,
