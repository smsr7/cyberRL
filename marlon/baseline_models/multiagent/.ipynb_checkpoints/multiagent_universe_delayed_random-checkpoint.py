...
from marlon.baseline_models.env_wrappers.defend_wrapper_delayed_random import DefenderEnvWrapper
...
class MultiAgentUniverse:
    ...
    def build(cls,
        ...
        defender_loss_reward: float = -7000.0,
        defender_maintain_sla: float = 0.70,
    ...
    def learn(self, total_timesteps: int, n_eval_episodes: int, ladder: bool = False, attacker_train: bool = False):
        '''
    ...
    def evaluate(self, n_episodes: int) -> EvalutionStats:
        ...
        mean_network_infected=[]
        std_network_infected=[]
        min_network_availability=[]
        std_network_availability=[]
        mean_network_availability=[]
        

        for i in range(n_episodes):
            self.logger.info(f'Evaluating episode {i+1} of {n_episodes}')
            episode_rewards1, episode_rewards2, defender_actions, _ = marl_algorithm.run_episode(
                attacker_agent=self.attacker_agent,
                defender_agent=self.defender_agent,
            ...
            if self.defender_agent:
                ...
                mean_network_infected.append(np.asarray(self.defender_agent.wrapper.network_infectivity).mean())
                std_network_infected.append(np.asarray(self.defender_agent.wrapper.network_infectivity).std())
                
                min_network_availability.append(np.asarray(self.defender_agent.wrapper.network_availability).min())
                std_network_availability.append(np.asarray(self.defender_agent.wrapper.network_availability).std())
                mean_network_availability.append(np.asarray(self.defender_agent.wrapper.network_availability).mean())
        ...
        stats = EvalutionStats(
            ...
            mean_network_infected=mean_network_infected,
            std_network_infected=std_network_availability,
            min_network_availability=min_network_availability,
            std_network_availability=std_network_availability,
            mean_network_availability=mean_network_availability
        )

        stats.log_results(self.logger)

        return stats, defender_actions
